...
const factionOptions = computed(() =>
  factions.value
    ...
    .map((f) => ({
      value: f.id,
      label: armyById(gameData, f.army)?.name ?? `势力${f.id}`
    }))
)

function planetName(id: number): string {
  return planetById(gameData, id)?.name ?? `星球#${id}`
}
...
<template>
  <div v-if="store.save">
    <el-alert
      ...
      show-icon
      :closable="false"
      title="修改所属势力会同步 FactionData.planets；乱改可能影响地图与剧情"
      class="warn"
    />
    <div class="toolbar">
      <el-button type="primary" @click="maxAllStats()">全部拉满（当前四维）</el-button>
      <span class="count">共 {{ planets.length }} 颗</span>
    </div>
    <el-table :data="planets" size="small" max-height="560">
      <el-table-column label="星球" min-width="120">
        <template #default="{ row }">{{ planetName(row.id) }}</template>
      </el-table-column>
      <el-table-column label="所属势力" min-width="180">
        <template #default="{ $index, row }">
          <el-select
            size="small"
            :model-value="row.faction"
            style="width: 100%"
            @change="(v) => onFaction($index, v)"
          >
            <el-option v-for="o in factionOptions" :key="o.value" :label="o.label" :value="o.value" />
          </el-select>
        </template>
      </el-table-column>
      <el-table-column label="经济" width="200">
        <template #default="{ $index, row }">
          <el-input-number
            size="small"
            ...
            @change="(v) => onStat($index, 'economics', v)"
          />
          <span class="max">/ {{ row.economicsMax }}</span>
        </template>
      </el-table-column>
      <el-table-column label="工业" width="200">
        <template #default="{ $index, row }">
          <el-input-number
            size="small"
            ...
            :max="row.industryMax"
            controls-position="right"
            @change="(v) => onStat($index, 'industry', v)"
          />
          <span class="max">/ {{ row.industryMax }}</span>
        </template>
      </el-table-column>
      <el-table-column label="防御" width="200">
        <template #default="{ $index, row }">
          <el-input-number
            size="small"
            ...
            :max="row.defenseMax"
            controls-position="right"
            @change="(v) => onStat($index, 'defense', v)"
          />
          <span class="max">/ {{ row.defenseMax }}</span>
        </template>
      </el-table-column>
      <el-table-column label="稳定" width="200">
        <template #default="{ $index, row }">
          <el-input-number
            size="small"
            ...
            controls-position="right"
            @change="(v) => onStat($index, 'stability', v)"
          />
          <span class="max">/ {{ row.stabilityMax }}</span>
        </template>
      </el-table-column>
      <el-table-column label="上限微调" min-width="220">
        <template #default="{ $index, row }">
          <div class="max-edit">
            <span>经</span>
...
.hint { font-size: 11px; color: #c0c4cc; margin-top: 2px; }
